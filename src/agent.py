...
class ReplayBuffer:
    ...
    def push(self, state: np.ndarray, action: int, reward: float,
        ...
        if not self.transitions or self.transitions[-1][3]:
...
class DQNAgent:
    ...
    def learn(self, batch: tuple[np.ndarray, ...]) -> tuple[float, float]:
        # Batch unpacking
        states, actions, rewards, next_states, dones = batch
        
        # Creation of torch tensors
        ...
        # Calculation of the current Q-value (Policy Net)
        current_q = self.policy_net(states).gather(1, actions)
        
        # Double DQN step: Calculation of Q-target
        with torch.no_grad():
            # The policy_net choose the best action
            best_next_actions = self.policy_net(next_states).argmax(dim=1, keepdim=True)
            
            # The target_net provides the value for the action chosen by policy_net
    ...
    # Update of the target network opy weights from policy network
